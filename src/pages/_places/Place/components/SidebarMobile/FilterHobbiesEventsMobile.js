...
class FilterHobbiesEvents extends React.Component {
  ...
  handleList = (id, active) => {
    const { dispatch, sideBar } = this.props;

    sideBar();
    dispatch(
      active ? clearSelectedSidebarHobby() : setSelectedSidebarHobby(id)
    );
  };

  render() {
    const { open } = this.state;
    const { dispatch, eventsHobbies, selectedHobby, location } = this.props;
          ...
          >
            <ul>
              {eventsHobbies.map(data => {
                ...
                return (
                  <li
                    className={classes(s.filterList, { [s.active]: active })}
                    key={data.id}
                    onClick={() => this.handleList(data.id, active)}
                  >
                    {data.name}
                  </li>
                );
...
export default withRouter(
  connect(mapStateToProps)(withStyles(s)(FilterHobbiesEvents))
);
