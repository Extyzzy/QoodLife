...
class ListContainer extends PaginatedTable {
  ...
  componentWillUnmount() {
    if (this.fetchProductDetailsFetcher instanceof Promise) {
      this.fetchProductDetailsFetcher.cancel();
    }
  ...
  fetchProductDetails() {
    ...
    this.fetchProductDetailsFetcher = fetchApiRequest(`/v1/products/${productId}`);

    this.fetchProductDetailsFetcher
      .then(response => {
  ...
  getLoadDataFetcher(filter) {
    ...
    return dispatch(
      fetchAuthorizedApiRequest(
        `/v1/products/${productId}/promotions${(queryData ? `?${queryData}` : '')}`,
        {
          headers: {
  ...
  deleteInterval(intervalId) {
    if (window.confirm(I18n.t('administration.confirmAction'))) {
      ...
      this.deleteIntervalFetcher = dispatch(
        fetchAuthorizedApiRequest(
          `/v1/products/promotions/${intervalId}`,
          {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${accessToken}`,
